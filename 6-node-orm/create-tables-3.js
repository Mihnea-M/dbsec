...
try {
  ...
  // additional unconnected examples
  let additionalResults = await Restaurant.findAll({
    attributes: ['openingHour', 'closingHour', [sequelize.fn('UCASE', sequelize.col('name')), 'upper_name']],
    raw: true
  })

  additionalResults = await sequelize.query('SELECT * FROM Locations', {
